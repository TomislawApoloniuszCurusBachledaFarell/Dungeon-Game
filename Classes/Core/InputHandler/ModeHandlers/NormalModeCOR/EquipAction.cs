...
public class EquipAction :NormalHandler
{
    public EquipAction() : base() { this.SetNext(new ExitAction()); }
    protected override bool canHandle(ConsoleKey PressedKey, KeyDefinitions keyBinds)
    {
        return PressedKey == keyBinds.GetActionKey(GameActions.Equip);
    }
    ...
    protected override InputIResult Process(ConsoleKey ckey, Player player, Maze maze, KeyDefinitions KeyBinds, ref InputMode inputMode, ref int? tempItemIndex)
    {
        ...
        if (player.HasEquipable())
        {
            ...
            result.resultMessage = InputMessages.EquipSuccess();
        }
        else
        {
            result.resultMessage = InputMessages.EquipFailure();
        }
        return result;
    }
}
